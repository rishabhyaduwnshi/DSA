...
class BinaryTree
{
    ...
    public:
        ...
        int getNodeCountOfTree(Node* root_node)
        {
            if(root_node != nullptr)
                return 1+ getNodeCountOfTree(root_node->left_child) + getNodeCountOfTree(root_node->right_child);
            return 0;
        }
};
...
int main()
{
   BinaryTree bt;
   bt.createTree();
   cout<<bt.getNodeCountOfTree(bt.getRootNode());
}
